...
// Helper: always returns a valid QStandardItem (never nullptr)
static QStandardItem *safeItem(QStandardItemModel *model, int row, int col)
{
...
void SuspiciousIPTable::updateRow(uint32_t ip, const DetectionResult &r)
{
    ...
    if (it == m_records.end())
    {
        // New IP — insert a fully populated row atomically
        int row = m_model->rowCount();

        // Insert all 6 cells at once so no cell is ever null
        QList<QStandardItem *> items;
        for (int c = 0; c < kHeaders.size(); ++c)
        {
        ...
        m_model->item(row, 0)->setText(QString::fromStdString(r.src_ip_str));
    ...
    // Use safeItem — guarantees no nullptr dereference
    safeItem(m_model, row, 1)->setText(QString::number(rec.hits));
    safeItem(m_model, row, 2)->setText(QString::number(rec.attackHits));
    safeItem(m_model, row, 3)->setText(QString::fromStdString(r.attack_type));
    safeItem(m_model, row, 4)->setText(QString("%1").arg(rec.maxScore, 0, 'f', 3));

    QString status = (r.severity == Severity::ATTACK)       ? "⬤ ATTACK"
                     : (r.severity == Severity::SUSPICIOUS) ? "◈ SUSPICIOUS"
                                                            : "○ NORMAL";
    QColor fg = (r.severity == Severity::ATTACK)       ? QColor("#f85149")
                : (r.severity == Severity::SUSPICIOUS) ? QColor("#ffa657")
                                                       : QColor("#3fb950");

    auto *statusItem = safeItem(m_model, row, 5);
